function [ I, weight_matrix, weight_matrix_STDP, tau_syn, wm_max, delay, par ] = create_network( par )
...
for g=1:length(par.nG)
    ...
    for i = 1:par.(['n_SG_' par.nG{g}])
        ...
        if(i > 1 && strcmp(par.nG{g}, 'TC') == 1) 
            ...
            [weight_matrix, delay, tau_syn, weight_matrix_STDP, wm_max, par] = ...
...
if(sum(strcmp('NC', par.nG)) > 0)
    for L = 1 : par.n_SG_NC
        if(strcmp(par.sim_type, 'recall') ~= 1)
            ...
            [I] = add_input(par.NC_stim_TS, stim_gamma, ...
                par.NC_stim_BG, par.NC_stim_W, par.NC_stims_t(L,:), ...
                par.NC_pre_stim_L, G1_E, par.NC_stim_N_cos, par.NC_stims_N(L,:), par, I );
        end
    end
end

%% TC INPUT TO KICK-START TIME CELLS
if(any(contains(par.nG,'TC')))
...
% NC <-> BP
if(any(contains(par.nG,'BP')) && any(contains(par.nG,'NC')))
...
if(any(contains(par.nG,'BP')) && any(contains(par.nG,'TC')))
    % TC <-> BP
...
function [weight_matrix, delay, tau_syn, weight_matrix_STDP, wm_max, par] = ...
    connect_neurons(N1, N2, R, TS, D, recipricol, W_ini, W_max, W_sigma, STDP, top_type, top_width,...
    weight_matrix, delay, tau_syn, weight_matrix_STDP, wm_max, par )
...
%% ADD SPIKES FUNCTION
function [I] = add_input(spike_ts, input_ts, rate, weight, stims, pre_stim, G_E, cos, N, par, I )
    %global par; global I;
